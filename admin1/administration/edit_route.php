...
<script>
  ...
  function getRoutes() {
    fetch("https://soshianest.rapidekops.us/admin/api/v1/get_menu_tree.php?key=text&type=json")
      .then((res) => res.json())
      .then((res) => {
        routes = res.sort((a, b) => Number(a.sort) - Number(b.sort));
  ...
  function updatedDatabaseNode(data) 
  {
    let parent        = data.parent == '#' ? true : data.parent;
    let old_position  = data.old_position
    let new_position  = data.position
    let old_parent    = data.old_parent == '#' ? 0 : data.old_parent
    let nodeId        = data.node.id
    let newParentId   = data.parent == '#' ? 0 : data.parent
   
    const formData = new FormData();
    formData.append("action", "edit_drag_menu");
    ...
    formData.append("old_parent", old_parent);
