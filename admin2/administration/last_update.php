...
<div class="container">
    ...
    <script>
        ...
        async function getPredictedData() {
            await fetch(`http://localhost/soshiane/admin2/api/v1/routes_data.php?action=get`)
                .then((res) => res.json())
                .then((res) => {
        ...
        async function getEvaluation() {
            await fetch(`http://localhost/soshiane/admin2/api/v1/routes_data.php?action=get_evaluation`)
        ...
        async function getTopTen() {
            await fetch(`http://localhost/soshiane/admin2/api/v1/top_ten.php?action=get_all`)
                .then(async (res) => await res.json())
                .then((res) => {
                    console.log(res);
       ...
       async function getRoutes() {
            fetch("http://localhost/soshiane/admin2/api/v1/get_menu_tree.php?key=text&type=json")
                .then((res) => res.json())
                .then(async (res) => {
                    routes = res;
                     getPredictedData()
        ...
        function parseData(data) {
            ...
            data.forEach(item => {
                ...
                result[parent.id].user_id = item.user_id ?? 'N/A';
                result[parent.id].user_name = item.user_name ?? 'N/A';
                processedRoutesId.push(route.id)
            ...
            const unhandledRoutes = routes.filter(item => {
                const route = routes.find(i => i.unique_id?.toString() === item.unique_id?.toString())
                return !processedRoutesId.includes(route.id) && route.period !== ""
            })
            unhandledRoutes.forEach(item => {
                const route = routes.find(i => i.unique_id?.toString() === item.unique_id?.toString())
                if (!route) return;
                const parent = findParent(route.parent)
                if (!parent) return;
                ...
                processedRoutesId.push(route.id)
                if (route.period.trim().toLowerCase() === "weekly") {
                    result[parent.id].W = "N/A"
                    result[parent.id].WDateModified = "N/A";
                    result[parent.id].isActiveW = String(route.is_active) === "1";
                }
                if (route.period.trim().toLowerCase() === "monthly") {
                    result[parent.id].M = "N/A"
                    result[parent.id].MDateModified = "N/A";
                    result[parent.id].isActiveM = String(route.is_active) === "1";
                }
                if (route.period.trim().toLowerCase() === "quarterly") {
                    result[parent.id].Q = "N/A"
                    result[parent.id].QDateModified = "N/A";
                    result[parent.id].isActiveQ = String(route.is_active) === "1";
                }
                if (route.period.trim().toLowerCase() === "annual") {
                    result[parent.id].A = "N/A";
                    result[parent.id].ADateModified = "N/A";
                    result[parent.id].isActiveA = String(route.is_active) === "1";
                }
                result[parent.id].isActive = String(parent.is_active) === "1";
            });
        ...
        function addRowToPredictionTable({
            ...
            const useId = document.createElement('td')
            useId.innerText = user_id;
