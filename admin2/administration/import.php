...
<script>
    ...
    async function importRequest() {
        ...
        console.log('all counts', page.data)
        let failedFiles = [];
    ...
    async function showUploadResult(failedFiles) {
        ...
        if (failedCount === 0) alert("success")
            ...
            const csvTable = document.getElementById('csvTable');
            csvTable.innerHTML = "";
            const csvData = ["Can't Send This "+failedCount+" Files to the server out of "+totalFiles+ " files"].concat(failedFiles).join("\n");
    ...
    function sendAverageRequest(formData) {
        return new Promise((resolve, reject) => {
            ...
            fetch("http://localhost/soshiane/admin2/api/v1/routes.php", {
                body: formData,
                method: "POST"
            })
                .then(async (res) => {
    ...
    function sendRequest(formData) {
        ...
        return new Promise((resolve, reject) => {
            $("#submit").attr("disabled", "disabled")
            fetch("http://localhost/soshiane/admin2/api/v1/routes_data.php", {
                body: formData,
                method: "POST"
            })
                .then(async (res) => {
